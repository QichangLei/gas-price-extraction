...
class PriceExtractor:
    ...
    def _iter_paddle_items(self, result):
        """
        Normalize PaddleOCR output across v2/v3 variants and yield raw line items.

        Common shapes:
          v2: result = [ [ [box, (text, conf)], ... ] ]
          v3: result = [ {text/rec_text/...}, TextResult objects, or [box,(text,conf)] ... ]
          v3 predict/paddlex: result may be object with .data
        """
        if result is None:
            return

        # Some wrappers return an object with .data
        if hasattr(result, "data"):
            result = result.data

        # v2 often nests a single "page": [page]
        if isinstance(result, list) and len(result) == 1 and isinstance(result[0], list):
            for item in result[0]:
                yield item
            return

        # Otherwise assume list of items
    ...
    def _parse_paddle_result(self, result) -> List[Dict]:
        """
        Parse PaddleOCR result into unified detection dicts:
          {text, confidence, bbox:[x,y,w,h], engine:'paddle'}
        Works across PaddleOCR 2.x and 3.x common formats.
        """
        detections: List[Dict] = []

        def poly_to_xywh(poly):
            # poly: [[x,y], [x,y], [x,y], [x,y]] or similar
            try:
        ...
        for item in self._iter_paddle_items(result):
            try:
                text = None
                conf = None
                bbox_xywh = None

                # Case A: dict outputs (some v3/pipeline shapes)
                if isinstance(item, dict):
                    text = item.get("text") or item.get("rec_text") or item.get("label")
                    conf = item.get("confidence") or item.get("rec_score") or item.get("score")

                    # bbox candidates
                    poly = item.get("text_box_position") or item.get("points") or item.get("dt_boxes")
                    bbox = item.get("bbox")

                    if bbox and isinstance(bbox, (list, tuple)) and len(bbox) == 4 and all(isinstance(v, (int, float)) for v in bbox):
                        bbox_xywh = [int(bbox[0]), int(bbox[1]), int(bbox[2]), int(bbox[3])]
                    elif poly and isinstance(poly, (list, tuple)) and len(poly) >= 4 and isinstance(poly[0], (list, tuple)):
                        bbox_xywh = poly_to_xywh(poly)

                # Case B: object outputs with attributes (TextResult-like)
                elif hasattr(item, "__dict__") and (hasattr(item, "text") or hasattr(item, "rec_text")):
                    ...
                    poly = getattr(item, "text_box_position", None)
                    if poly is None:
                        poly = getattr(item, "dt_boxes", None)
                    if poly and isinstance(poly, (list, tuple)) and len(poly) >= 4 and isinstance(poly[0], (list, tuple)):
                        bbox_xywh = poly_to_xywh(poly)

                # Case C: classic v2 line: [poly, (text, conf)]
                ...
                # If bbox missing, keep a placeholder (won't crash scoring)
                ...
                # Per-item robustness
                if self.debug:
                    print(f"[WARN] Paddle item parse failed: {e} | item type={type(item)}")
                continue
    ...
    def extract_with_paddle(self, img: np.ndarray) -> List[Dict]:
        ...
        try:
            ...
            # Parse (unified)
            detections = self._parse_paddle_result(result)
            return detections
