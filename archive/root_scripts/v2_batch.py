#!/usr/bin/env python3
"""
Gas Price Extraction with Multiple OCR Engines - Enhanced for Multiple Prices and Batch Processing
...
Usage:
    # Single image
    python v2_mul_batch.py <image_path> [--engine paddle|easy|all] [--debug]
    
    # Process entire directory
    python v2_mul_batch.py <directory_path> [--engine paddle|easy|all] [--output-dir results]
"""
...
class PriceExtractor:
    ...
    def extract_all_prices(self, image_path: str) -> PriceResult:
        ...
        print(f"[INFO] Processing image: {Path(image_path).name} ({img.shape[1]}x{img.shape[0]})")
        ...
        # Extract all valid prices and combine duplicates
        price_detections = []
        seen_prices = {}  # Map (price, position) -> detection with best confidence
        
        for det in all_detections:
            price = self.parse_price(det['text'])
            if price is not None:
                # Check if we already have this price at roughly this position
                x, y, w, h = det['bbox']
                pos_key = (round(x / 20), round(y / 20))  # Group positions within 20px
                price_key = (price, pos_key)
                
                if price_key not in seen_prices:
                    # First detection of this price at this position
                    det['price'] = price
                    det['engines'] = [det['engine']]
                    det['confidences'] = [det['confidence']]
                    seen_prices[price_key] = det
                else:
                    # Duplicate detection - merge with existing
                    existing = seen_prices[price_key]
                    existing['engines'].append(det['engine'])
                    existing['confidences'].append(det['confidence'])
                    
                    # Update to use the highest confidence
                    if det['confidence'] > existing['confidence']:
                        existing['confidence'] = det['confidence']
                        existing['engine'] = det['engine']
                        existing['text'] = det['text']
                        existing['bbox'] = det['bbox']
                        existing['score'] = det['score']
        
        price_detections = list(seen_prices.values())
        ...
        for group_idx, group in enumerate(grouped_prices):
            for det in group:
                ...
                # Format engine info
                engines_list = det.get('engines', [det['engine']])
                if len(engines_list) > 1:
                    engine_str = f"{det['engine']} (+{', '.join(set(engines_list) - {det['engine']})})"
                    num_detections = len(engines_list)
                else:
                    engine_str = det['engine']
                    num_detections = 1
        ...
        result_prices.sort(key=lambda x: (-x['confidence'], x['bbox'][1]))
        ...
        for i, p in enumerate(result_prices, 1):
            detection_note = f" [detected by {p['num_detections']} engine(s)]" if p['num_detections'] > 1 else ""
            print(f"  {i}. ${p['price']:.3f} - {p['fuel_type']}{detection_note}")
            print(f"     (text='{p['raw_text']}', engine={p['engine']}, conf={p['confidence']:.2%})")
...
def save_result_to_file(result: dict, filepath: str, format_type: str = 'txt'):
    """
    Save extraction results to a file.
    
    Args:
        result: Result dictionary from extract_price()
        filepath: Output file path
        format_type: Output format ('txt', 'json', or 'csv')
    """
    import json
    from pathlib import Path
    ...
    if format_type == 'json':
        ...
        # Save as CSV
        with open(filepath, 'w', encoding='utf-8') as f:
            if result.get('ok') and result.get('prices'):
                # Header
                f.write("Price,Fuel Type,Confidence,Engine,Num Detections,Raw Text,BBox X,BBox Y,BBox W,BBox H\n")
                
                # Data rows
                for price_info in result['prices']:
                    bbox = price_info.get('bbox', [0, 0, 0, 0])
                    f.write(f"{price_info['price']:.3f},")
                    f.write(f"\"{price_info.get('fuel_type', 'Unknown')}\",")
                    f.write(f"{price_info['confidence']:.4f},")
                    f.write(f"\"{price_info['engine']}\",")
                    f.write(f"{price_info.get('num_detections', 1)},")
                    f.write(f"\"{price_info['raw_text']}\",")
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write("=" * 60 + "\n")
            f.write("GAS PRICE EXTRACTION RESULTS\n")
            f.write("=" * 60 + "\n\n")
            
            if result.get('ok'):
                if result.get('prices'):
                    f.write(f"Found {len(result['prices'])} price(s) (sorted by confidence):\n\n")
                    
                    for i, price_info in enumerate(result['prices'], 1):
                        detection_note = ""
                        if price_info.get('num_detections', 1) > 1:
                            detection_note = f" [detected by {price_info['num_detections']} engines]"
                        
                        f.write(f"{i}. ${price_info['price']:.3f} - {price_info.get('fuel_type', 'Unknown')}{detection_note}\n")
                        f.write(f"   Raw text: '{price_info['raw_text']}'\n")
                        f.write(f"   Confidence: {price_info['confidence']:.2%}\n")
                        f.write(f"   Engine: {price_info['engine']}\n")
                        
                        bbox = price_info.get('bbox', [0, 0, 0, 0])
                        f.write(f"   Position: x={bbox[0]}, y={bbox[1]}, w={bbox[2]}, h={bbox[3]}\n")
                        f.write("\n")
                else:
                    f.write(f"Price: ${result['price']}\n")
                    f.write(f"Raw Text: '{result.get('raw_text', 'N/A')}'\n")
                    f.write(f"Confidence: {result.get('confidence', 0):.2%}\n")
                    f.write(f"Engine: {result.get('engine', 'N/A')}\n")
            else:
...
def process_directory(dir_path: str, engine: str = "all", debug: bool = False, 
                     output_dir: str = None, format_type: str = "txt") -> dict:
    """
    ...
    Args:
        ...
        format_type: Output format for individual results
        
    Returns:
        Dictionary with summary of all processed images
    """
    import os
    from pathlib import Path
    ...
    if not image_files:
        return {"ok": False, "error": f"No image files found in {dir_path}"}
    ...
    for idx, image_file in enumerate(image_files, 1):
        ...
        result = extract_price(str(image_file), engine=engine, debug=debug)
        ...
        if result.get('ok'):
            ...
            if output_dir:
                output_file = output_path / f"{image_file.stem}_result.{format_type}"
                save_result_to_file(result, str(output_file), format_type)
        else:
            failed += 1
    ...
    if output_dir:
        summary_file = output_path / f"summary.json"
        import json
...
def print_summary(summary: dict):
    ...
    print(f"Success rate: {summary['successful']/summary['total_images']*100:.1f}%")
    ...
    for result in summary['results']:
        ...
        if result.get('ok'):
            if result.get('prices'):
                print(f"\n✓ {filename}: {len(result['prices'])} price(s) found")
                for i, price_info in enumerate(result['prices'][:3], 1):  # Show top 3
                    fuel = price_info.get('fuel_type', 'Unknown')
                    print(f"  {i}. ${price_info['price']:.3f} - {fuel} ({price_info['confidence']:.1%})")
                if len(result['prices']) > 3:
                    print(f"  ... and {len(result['prices']) - 3} more")
            else:
                print(f"\n✓ {filename}: ${result.get('price', 0):.3f}")
        else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Extract gas prices from images (single file or entire directory)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Process single image
  python v2_mul_batch.py image.jpg
  
  # Process all images in directory
  python v2_mul_batch.py /path/to/images/
  
  # Process directory and save results
  python v2_mul_batch.py /data/price_1/ --output-dir ./results --format json
  
  # Process with specific engine
  python v2_mul_batch.py /data/price_1/ --engine paddle --debug
        """
    )
    ...
    parser.add_argument("--output", "-o", help="Output file path for single image (supports .txt, .json, .csv)")
    ...
    if path.is_dir():
        ...
        # Process single file
        result = extract_price(str(path), engine=args.engine, debug=args.debug)
        ...
        if args.output:
            save_result_to_file(result, args.output, output_format or 'txt')
        ...
        if result.get('ok'):
            if result.get('prices'):
                print(f"\n  Found {len(result['prices'])} price(s) (sorted by confidence):\n")
                for i, price_info in enumerate(result['prices'], 1):
                    detection_note = ""
                    if price_info.get('num_detections', 1) > 1:
                        detection_note = f" [detected by {price_info['num_detections']} engines]"
                    
                    print(f"  {i}. ${price_info['price']:.3f} - {price_info.get('fuel_type', 'Unknown')}{detection_note}")
